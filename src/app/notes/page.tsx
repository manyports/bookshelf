...
import { Book, Plus, Edit, Trash, Save, X, Search, Bold, Hash, PenTool } from 'lucide-react';
...
export default function BookNotes() {
  ...
  const updateNoteSpace = () => {
    setNoteSpaces(noteSpaces.map(space => 
  ...
  const filteredNotes = activeSpace 
    ? noteSpaces.find(space => space.id === activeSpace)?.notes.filter(note => 
  ...
  const activeNoteData = activeSpace && activeNote 
    ? noteSpaces.find(space => space.id === activeSpace)?.notes.find(note => note.id === activeNote)
    : null;
    ...
    <div className="container mx-auto p-4">
      ...
      <div className={`flex flex-col ${isMobile ? '' : 'md:flex-row'} gap-6`}>
        <Card className={`w-full ${isMobile ? '' : 'md:w-1/4'}`}>
          ...
          <CardContent>
            <ScrollArea className={`${isMobile ? 'h-[200px]' : 'h-[calc(100vh-300px)]'}`}>
              <Tabs value={activeSpace || undefined} onValueChange={setActiveSpace}>
                <TabsList className="flex flex-col items-stretch">
                  {noteSpaces.map(space => (
                    <TabsTrigger key={space.id} value={space.id} className="justify-between">
                      {isEditingSpace && activeSpace === space.id ? (
                        <Input
                          value={newSpaceName}
                          onChange={(e) => setNewSpaceName(e.target.value)}
                          onBlur={updateNoteSpace}
                          onKeyPress={(e) => e.key === 'Enter' && updateNoteSpace()}
                          className="w-full"
                        />
                      ) : (
                        <>
                          ...
                          <div>
                            ...
                            <Button variant="ghost" size="icon" onClick={() => deleteNoteSpace(space.id)}>
          ...
          <Card className={`w-full ${isMobile ? '' : 'md:w-3/4'}`}>
            ...
            <CardContent>
              <div className={`flex ${isMobile ? 'flex-col' : ''} gap-6`}>
                <ScrollArea className={`${isMobile ? 'h-[200px]' : 'h-[calc(100vh-300px)]'} ${isMobile ? 'w-full' : 'w-1/3'}`}>
                  {filteredNotes?.map(note => (
                    <Card key={note.id} className="mb-4 cursor-pointer hover:bg-gray-100" onClick={() => setActiveNote(note.id)}>
                      <CardHeader>
                        <CardTitle className="flex justify-between items-center">
                          {note.title}
                      ...
                      <CardContent>
                        ...
                        <div className="flex flex-wrap gap-1 mt-2">
                          {note.tags.map(tag => (
                            <span key={tag} className="bg-gray-200 text-gray-800 text-xs font-semibold px-2.5 py-0.5 rounded">
                              {tag}
                            </span>
                          ))}
                        </div>
                ...
                {activeNoteData && (
                  <div className={`${isMobile ? 'w-full' : 'w-2/3'}`}>
                    <Input
                      ...
                      onChange={(e) => updateNote(activeNoteData.id, e.target.value, activeNoteData.content)}
                      className="mb-4"
                    />
                    <MenuBar editor={editor} />
                    <EditorContent editor={editor} className="prose max-w-none mb-4 p-4 border rounded-md min-h-[300px]" />
                    <div className="flex justify-between items-center">
                      <div className="flex flex-wrap gap-1">
                        {activeNoteData.tags.map(tag => (
                          <span key={tag} className="bg-gray-200 text-gray-800 text-xs font-semibold px-2.5 py-0.5 rounded">
                            {tag}
                            <button onClick={() => removeTag(tag)} className="ml-1 text-gray-500">&times;</button>
                          </span>
                        ))}
                      </div>
